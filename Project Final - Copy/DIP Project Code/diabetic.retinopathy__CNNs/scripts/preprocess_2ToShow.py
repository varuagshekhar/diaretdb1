...
BLOB_PARAMS.filterByInertia = True #For the consistent positioning and shape of the notch
...
def preprocess_image(path):
    ...
    if detect_notch(img, img_thresh):
        ...
        print(here)
        plt.subplot(164)
        plt.imshow(draw_hough_circles(img, here))
...
path = "../diaretdb0_v_1_1/resources/images/diaretdb0_fundus_images/image" + number+".png"

#85, 
path_final = cv.imread("../normalised_images/image" + number + ".png")
path_pre = cv.imread(path)
path_pre = cv.cvtColor(path_pre, cv.COLOR_BGR2RGB)
...
plt.subplot(166)
plt.imshow(path_final)
plt.title("Final Greyscale")
plt.show()
